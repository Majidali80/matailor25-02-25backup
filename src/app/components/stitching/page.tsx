...
import ToggleAddToCart from '../ToggleAddToCart'; // Adjust the import path as necessary
...
export default function Home() {
  ...
  const handleToggle = () => {
    setIsAdded(!isAdded);
    console.log("Toggle state:", !isAdded);
    ...
    <div className="container mx-auto px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {products.map((product) => {
          const discountPercentage = product.discountPercentage ?? 0; // Default to 0 if undefined
          ...
          return (
            <div key={product._id} className="relative">
              ...
              <button
                onClick={() => {
                    const cartItem: CartItem = {
                        ...
                        productImage: product.productImage, // Ensure this matches your CartItem type
                        customization: {
                            designType: "Default Design", // Provide a default value or use actual data
                            fabricType: "Default Fabric", // Provide a default value or use actual data
                            measurements: {
                                chest: 0, // Provide default values
                                waist: 0,
                                hips: 0,
                                length: 0,
                                armHole: 0,
                    ...
                    addToCart(cartItem); // Pass the constructed CartItem
                }}
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center">
                {/* Availability */}
                <div className="text-sm text-gray-600">{product.availability}</div>
                <div className="text-sm text-gray-600">{product.isNewArrival}</div>

                {/* Discounted Price */}
                <div className="font-bold text-red-500">PKR {discountedPrice.toFixed(2)}</div>
              </div>
              <div className="text-sm text-gray-600">{product.discountPercentage}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* Add the ToggleAddToCart component at the bottom */}
      <ToggleAddToCart isAdded={isAdded} onToggle={handleToggle} itemCount={itemCount} />
    </div>
  );
}
